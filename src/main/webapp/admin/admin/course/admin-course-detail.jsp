...
<script>
    ...
    async function loadCourseLazyPanel(name) {
        ...
        panel.innerHTML = '<div class="cd-surface px-22 py-22 text-center text-14 text-neutral-500">Loading…</div>';
        try {
    ...
    async function loadCourseClassGroupActivities(panel, trigger) {
        ...
        panel.innerHTML = '<div class="border-top border-neutral-30 mt-12 pt-12 text-13 text-neutral-500">Loading activities…</div>';
        try {
            var response = await fetch(withCurrentCourseSession(url), {
                method: 'GET',
                credentials: 'same-origin',
    ...
    async function loadCourseSubjectGroups(panel, trigger) {
        ...
        panel.setAttribute('aria-busy', 'true');
        setCourseControlLoading(trigger, true);
        panel.innerHTML = '<div class="text-13 text-neutral-500">Loading class groups…</div>';
        try {
            var response = await fetch(withCurrentCourseSession(url), {
                method: 'GET',
                credentials: 'same-origin',
    ...
    function configureCourseOccurrenceForm(scope) {
        ...
        function renderAcademicYears() {
            ...
            if (pickerTitle) {
                pickerTitle.textContent = pageStart + ' – ' + (pageStart + 11);
            }
